...
export function AppSidebar() {
    ...
    try {
      ...
      if (user) {
        ...
        if (avatarFile && avatarFile.length > 0) {
          const {
            data
          } = supabase.storage.from('avatars').getPublicUrl(`${user.id}/${avatarFile[0].name}`);
  ...
  return <Sidebar collapsible="icon" className="border-r border-border/40" style={{
    borderRadius: '10px',
    ...
    boxShadow: '0 4px 20px 0 rgba(229, 237, 247, 0.30)'
      ...
      <SidebarHeader className="border-b border-border/40 px-3 py-4">
        <div className="flex items-center gap-2">
          <img src="/lovable-uploads/311a49a3-a023-4208-954c-dad2347c5e50.png" alt="Thaski Logo" className="h-8" />
        </div>
      </SidebarHeader>
      ...
      <SidebarFooter className="border-t border-border/40 px-3 py-4 space-y-4">
        ...
        <Button variant="ghost" onClick={handleSignOut} className="w-full justify-start gap-3 px-3 py-2 h-auto text-muted-foreground hover:text-foreground hover:bg-muted/50">
